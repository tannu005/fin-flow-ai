...
import { Search, RefreshCw, BarChart3, LayoutDashboard, Bookmark, Download, FileText, Activity, TrendingUp, TrendingDown, ChevronRight, Zap, Code, Terminal, Bell, Filter, Grid, List, Sparkles, Calendar, Info, BrainCircuit, Target, Database, Minus, X, BarChart2, ChevronDown, AlertTriangle } from 'lucide-react';
...
const API_URL = typeof window !== 'undefined' && window.location.hostname.includes('vercel.app') ? '/_backend/api' : import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
...
const TrajectoryChart = ({
  ...
  return <div className="relative w-full h-32 mt-6">
      ...
      <div className="flex justify-between mt-2 text-[8px] font-black text-slate-600 uppercase tracking-widest">
        ...
        return <span key={i} className={i === data.length - 1 ? "text-rose-900 animate-pulse" : ""}>
...
const MarketTicker = ({
  pulse
    ...
    <div className="animate-ticker flex whitespace-nowrap">
      {(pulse?.indices || LIVE_INDICES).map((idx, i) => <div key={i} className="flex items-center gap-6 mx-10 shrink-0">
    ...
    <div className="absolute right-6 bottom-1 text-[9px] font-black text-rose-900 tracking-widest uppercase opacity-40">
      Live Pulse: {new Date().toLocaleDateString('en-US', {
      month: 'long',
      day: 'numeric',
      year: 'numeric'
...
export default function App() {
  ...
  } = useSWR(`${API_URL}/market-pulse`, fetcher, {
    refreshInterval: 30000
  });
  const {
    data: marketHistoryRaw
  } = useSWR(`${API_URL}/market-history`, fetcher);
  const {
    data: summariesRaw,
    mutate: mutateSummaries,
    isValidating: summariesValidating
  } = useSWR(`${API_URL}/summaries?date=${selectedHistory}`, fetcher);
  const {
    data: health
  } = useSWR(`${API_URL}/health`, fetcher);
  const {
    data: intelligenceRaw
  } = useSWR(`${API_URL}/platform-intelligence`, fetcher, {
  ...
  const trendingTopics = useMemo(() => {
    ...
    return Object.entries(topics).sort((a, b) => b[1] - a[1]).slice(0, 10).map(t => ({
      topic: t[0],
      count: t[1]
    }));
  }, [summaries, search]);
  ...
  const handleScrape = async () => {
    ...
    const initialLogs = [{
      ...
      msg: "📡 Initializing Engine (v4)..."
    }, {
      time: new Date().toLocaleTimeString(),
      msg: "🕵️ Bypassing security layers..."
    }, {
      time: new Date().toLocaleTimeString(),
      msg: "🔄 Rotating Proxy: Secure Gateway..."
    }];
    let i = 0;
  ...
  const filtered = summaries.filter(s => {
    ...
    const headline = s.headline || '';
    const summary = s.summary || '';
    const matchesSearch = headline.toLowerCase().includes(search.toLowerCase()) || summary.toLowerCase().includes(search.toLowerCase());
  ...
  return <div className="flex h-screen overflow-hidden text-slate-900 bg-slate-50 font-inter relative">
      <Background3D color="#3f0f1f" />

      {/* Luxury Notification Bar */}
      {notification && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[9999] px-6 py-3 rounded-full bg-white border border-slate-200 shadow-2xl backdrop-blur-xl flex items-center gap-3 animate-in slide-in-from-top-10 fade-in duration-300">
      ...
      <aside className="w-72 flex flex-col shrink-0 z-50 bg-rose-950 border-r border-rose-900 shadow-2xl text-white">
        ...
        <nav className="flex-1 px-4 py-8 space-y-2">
          ...
          label: 'History Vault',
          icon: Calendar
        }].map(item => <button key={item.id} onClick={() => setView(item.id)} className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-2xl text-sm font-bold transition-all ${view === item.id ? 'bg-slate-50 text-rose-900 border border-rose-800/30 shadow-lg shadow-rose-900/20' : 'text-slate-600 hover:text-slate-800 hover:bg-white'}`}>
          ...
          <div className="pt-8 space-y-2 px-4">
            ...
            <div className="relative group">
              <select value={selectedHistory} onChange={e => handleHistorySelect(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-700 text-xs font-bold py-3 px-4 rounded-xl appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-rose-800">
                <option value="Today">Today ({new Date().toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric'
                })})</option>
                {marketHistory.slice(0, -1).reverse().map(h => <option key={h.date} value={h.date}>{h.date}</option>)}
              </select>
          ...
          <div className="pt-8 space-y-2">
            ...
            {['All', 'Markets', 'Tech', 'Economy', 'Crypto'].map(cat => <button key={cat} onClick={() => {
            setCategory(cat);
            setView('dashboard');
      ...
      <main className="flex-1 flex flex-col relative z-40 overflow-hidden">
        <header className="h-20 px-10 flex items-center gap-8 shrink-0 bg-white backdrop-blur-xl border-b border-slate-200">
          <div className="flex-1 relative max-w-xl">
            ...
            <input type="text" placeholder="Query Market Narratives..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white border border-slate-200 py-3.5 pl-12 pr-4 rounded-2xl text-sm font-medium text-slate-800 placeholder-rose-300 focus:outline-none focus:border-rose-800 transition-all shadow-sm" />
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar dashboard-content">
          <div className="max-w-[1500px] mx-auto">
            {isMarketStressed && <div className="reveal-up mb-6 p-4 rounded-2xl bg-rose-50 border border-rose-200 flex items-center justify-between animate-pulse">
              ...
              <div className="reveal-up liquid-glass p-8 bg-white border-slate-200 flex flex-col justify-between">
                <div>
                  <h3 className="font-bold text-xs flex items-center gap-2 text-slate-800 uppercase tracking-widest mb-4">
                    <Zap size={16} className="text-slate-9000" /> {'Viral Signals'}
                  </h3>
                ...
                <div className="mt-8 pt-6 border-t border-slate-200">
                  ...
                  <div className="flex items-center gap-2 text-rose-900">
                    <Activity size={14} />
                    <span className="text-xs font-black uppercase">{'Geopolitical Stress'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              color: 'text-slate-9000',
              bg: 'bg-rose-800/10'
            }].map((stat, i) => <div key={i} className={`reveal-up bg-white backdrop-blur-lg border p-6 rounded-[32px] shadow-sm hover:shadow-xl transition-all group border-slate-200 ${isMarketStressed && stat.label === 'Volatility (VIX)' ? 'border-rose-800/30 shadow-[0_0_15px_rgba(244,63,94,0.15)]' : ''}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
              ...
              return <div key={sector} className={`reveal-up p-8 liquid-glass transition-all duration-500 ${isMatch ? 'bg-slate-50 border-rose-800/50 shadow-[0_0_30px_rgba(59,130,246,0.15)] ring-1 ring-rose-800/50 scale-[1.02]' : 'bg-white border-slate-200 opacity-80'}`}>
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between">
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${isMatch ? 'text-rose-900' : 'text-slate-500'}`}>{new Date().toLocaleDateString('en-US', {
                      month: 'long',
                      day: 'numeric'
                    })} Insight</span>
            ...
            {view === 'dashboard' && <div className="reveal-up mb-8 p-8 liquid-glass bg-gradient-to-br from-white to-rose-50 text-slate-900 border-none overflow-hidden relative">
                ...
                <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[{
                  trigger: 'Scenario: Fed Rate Cut',
                  impact: 'Bullish for Tech / Crypto',
                  probability: '65%'
                }, {
                  trigger: 'Scenario: Hormuz De-escalation',
                  impact: 'Bearish for Oil / Defensive',
                  probability: '42%'
                }, {
                  trigger: 'Scenario: Sovereign AI Mandate',
                  impact: 'Bullish for Local Infrastructure',
                  probability: '88%'
                }].map((sim, i) => <div key={i} className="p-4 rounded-2xl bg-white hover:bg-white transition-all cursor-pointer group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
              {loading ? Array(6).fill(0).map((_, i) => <div key={i} className="h-96 rounded-[40px] skeleton opacity-50" />) : view === 'saved' || filtered.length > 0 ? (/* Dynamic Feed / History Vault Feed */
            <>
                  {view === 'saved' && <div className="col-span-full mb-4 p-6 bg-white text-slate-900 rounded-[32px] flex items-center justify-between">
                  ...
                  {filtered.map((article, i) => <SummaryCard key={i} article={article} index={i} meta={pulse?.metadata} isStressed={isMarketStressed} />)}
      ...
      {showLogs && <div className="fixed inset-x-0 bottom-0 z-[9999] p-6 pointer-events-none">
          <div className="max-w-xl ml-auto bg-white backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl p-6 pointer-events-auto overflow-hidden">
            ...
            <div className="space-y-2 font-mono text-[11px] max-h-48 overflow-y-auto custom-scrollbar pr-2">
              {logs.map((log, i) => <div key={i} className="text-slate-700 border-l border-slate-200 pl-3 py-1 animate-in fade-in slide-in-from-left-2 duration-300">
                  {log}
                </div>)}
              {loading && <div className="flex items-center gap-2 text-rose-900 pl-3 py-1 mt-2">
